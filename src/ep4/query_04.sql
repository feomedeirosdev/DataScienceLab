WITH        tb_idade_base AS (
    ...
    SELECT      t2.seller_id,
                CAST(max(julianday('{date}') - julianday(t1.order_approved_at)) AS INT) AS idade_base_dias
...
SELECT      '{date}' AS dt_ref,
            ...
            CAST(julianday('{date}') - julianday(max(t1.order_approved_at)) AS INT) AS qtd_dias_ultima_venda, -- dias da última venda
...
WHERE       order_approved_at BETWEEN DATE('{date}', '-6 months') AND '{date}'
